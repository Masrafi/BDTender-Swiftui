...
struct PrivateTendesDetails: View {
    @AppStorage("savedEmail") var savedEmail: String = ""
    @State var items: [PrivateTenderModel] = []
    ...
    var body: some View {
        ScrollView (.vertical){
            ...
            AF.request("https://www.bdtender.com/api/own-tender-notice", method: .get).validate(statusCode: 200..<300)
                .responseString(completionHandler: {
                str in
                print("str: ", str)
            })
                .responseDecodable(of: [PrivateTenderModel].self, decoder: decoder) {
                response in
...
struct PrivateTendesDetails_Previews: PreviewProvider {
    static var previews: some View {
        PrivateTendesDetails()
    }
}
