...
struct LiveTendesDetails: View {
    @AppStorage("savedEmail") var savedEmail: String = ""
    @State var items: [LiveTenderModel] = []
    ...
    var body: some View {
        ScrollView (.vertical){
            ...
            AF.request("https://www.bdtender.com/api/live-tender", method: .get).validate(statusCode: 200..<300)
                .responseString(completionHandler: {
                str in
                print("str: ", str)
            })
                .responseDecodable(of: [LiveTenderModel].self, decoder: decoder) {
                response in
...
struct LiveTendesDetails_Previews: PreviewProvider {
