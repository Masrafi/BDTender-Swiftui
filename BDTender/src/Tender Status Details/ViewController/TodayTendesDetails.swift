...
struct TodayTendesDetails: View {
    @AppStorage("savedEmail") var savedEmail: String = ""
    @State var items: [TodayTenderModel] = []
    ...
    var body: some View {
        ScrollView (.vertical){
            ...
            AF.request("https://www.bdtender.com/api/today-tender", method: .get).validate(statusCode: 200..<300)
                .responseString(completionHandler: {
                str in
                print("str: ", str)
            })
                .responseDecodable(of: [TodayTenderModel].self, decoder: decoder) {
                response in
...
struct TodayTendesDetails_Previews: PreviewProvider {
